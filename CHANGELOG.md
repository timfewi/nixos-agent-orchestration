...
- `modules/shell.nix`: **tmux** (`tentaflake.shell.tmux.enable`, configured) and **kitty** (`tentaflake.shell.kitty.enable`, GUI terminal that auto-starts tmux via `KITTY_WINDOW_ID` in the shell init; implies tmux). Convenience aliases `rebuild` (→ `nixos-rebuild switch --flake /etc/nixos#<host>`), `reload` (`exec $SHELL`) and `cls`. tmux moved out of the `tools` package set into its own toggle.
